...
export class Demonetvis {
  ...
  async display() {
    console.log(this.tensorActivation2)
    ...
    this.activationPlane1.children[0].material.map = await tensorTexture(tf.reshape(tf.slice(this.tensorActivation1, [0, 0, 0, 0], [-1, 3, -1, -1]), [222, 222, 3]))
    this.activationPlane1.children[0].material.needsUpdate = true
    this.activationPlane2.children[0].material.map = await tensorTexture(tf.slice(this.tensorActivation2, [0, 0, 0, 0], [-1, 3, -1, -1]))
